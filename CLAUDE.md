...
This provider pins **NSchema.Core 3.1.0**, which added the column's final type/nullability to `AlterColumnType` / `AlterColumnNullability` (SQL Server's `ALTER COLUMN` must restate both at once). When iterating on Core alongside this repo, rebuild and re-push the Core package to the local cache after any change:
